...
invisible(tryCatch(credentials::ssh_key_info(), error = function(e){
...
The `credentials` package provides a few utility functions to make this easier. The `ssh_key_info()` function calls out to looks up which key that `ssh` uses to connect to a given server. This is usually `~/.ssh/id_rsa` unless you have a fancy custom ssh configuration.
